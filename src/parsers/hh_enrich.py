...
_TIMEOUT = 20
_PAUSE = 0.25          # между запросами: 56 страниц за прогон — не долбёжка
...
def enrich(jobs: Iterable[Job], limit: int = 120) -> EnrichStats:
    ...
    if not stats.healthy:
        logger.error(
            "HH дообогащение: извлечено только %.0f%% (%d из %d) — похоже, "
            "сменилась вёрстка hh.ru. Проверь селекторы в hh_enrich._SELECTORS.",
