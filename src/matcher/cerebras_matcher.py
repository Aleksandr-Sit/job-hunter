    ...
    for p in providers:
        try:
            resp = _get_client(p).chat.completions.create(
                ...
                temperature=0.4,
                # 600 -> 1400 (27.08.2026). Модель gpt-oss-120b тратит часть бюджета
                # выходных токенов на внутренние рассуждения, и при 600 письмо
                # обрывалось на полуслове — молча, без ошибки в логе. Поймано после
                # синхронизации профиля: он вырос с 17 326 до 20 376 символов, рассуждений
                # стало больше, и на текст перестало хватать. Письмо генерируется по
                # кнопке, одно за раз, поэтому запас дешевле обрыва.
                max_tokens=1400,
                timeout=_CEREBRAS_TIMEOUT,
            )
