...
Кнопки с `url` работают без дополнительной логики. Кнопки с `callback_data` требуют
обработчика в `src/bot/callback_handler.py` (он уже запущен фоновым потоком из
`scheduler.py` и слушает нажатия через polling) — для новой кнопки добавь там
`CallbackQueryHandler` с своим паттерном `callback_data`, иначе нажатие ничего не сделает.
